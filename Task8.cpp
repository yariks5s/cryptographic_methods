...
int main()
{
    ...
    do
    {
        std::cout << "Введіть просте число p :" << std::endl;
        std::cin >> p;
        flag = isPrime(p);

        if (!flag)
            std::cout << "Неправильний ввід" << std::endl;
    }
    while (!flag);

    do
    {
        std::cout << "Введіть просте число q :" << std::endl;
        std::cin >> q;
        flag = isPrime(q);
    ...
    while (!flag); //key * e = 1 mod(func)

    n = p * q;
    std::cout << "\nРезультат обчислення n = p*q = " << n << std::endl;

    t = (p - 1) * (q - 1);
    std::cout << "Результат обчислення функції Ейлера: " << t << std::endl;
    ...
    std::cout << "\nТекст повідомлення: " << message << std::endl;
